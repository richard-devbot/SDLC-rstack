...
test('stableStringify drops server eval-time timestamps so the ETag is stable across rebuilds', () => {
  ...
  const buildA = { ts: '2026-06-14T09:00:00.000Z', policy: { loadedAt: '2026-06-14T09:00:00.000Z', runBudgetUsd: 10 }, runs: [{ id: 'r1', status: 'DONE' }], alerts: [{ kind: 'stalled', ts: 1 }], studio: { freshness: { state: 'fresh', age_ms: 1000 } }, decisions: { runs: [{ readiness: { generated_at: '2026-06-14T09:00:00.001Z', status: 'PASS' } }] } };
  const buildB = { ts: '2026-06-14T09:00:05.000Z', policy: { loadedAt: '2026-06-14T09:00:05.000Z', runBudgetUsd: 10 }, runs: [{ id: 'r1', status: 'DONE' }], alerts: [{ kind: 'stalled', ts: 2 }], studio: { freshness: { state: 'fresh', age_ms: 6000 } }, decisions: { runs: [{ readiness: { generated_at: '2026-06-14T09:00:05.999Z', status: 'PASS' } }] } };
