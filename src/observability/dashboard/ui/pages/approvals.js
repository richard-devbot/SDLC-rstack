...
function opsAuditRejectionHtml(item) {
  ...
  return '<div class="alert-card critical"><div class="agent-head"><div>' +
    ...
    '<div class="feed-meta">' + (d.record_id ? '<span>record ' + esc(d.record_id) + '</span>' : '') + (d.status ? '<span>claimed status: ' + esc(d.status) + '</span>' : '') + '<span>' + esc((item.runId || '').slice(-16)) + '</span><span>' + timeHtml(item.ts) + '</span></div>' +
