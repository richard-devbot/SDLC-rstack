...
Installs the `sdlc-rstack` plugin from `plugins/sdlc-rstack/` — one slash
command per `sdlc_*` tool (`/sdlc-start`, `/sdlc-plan`, `/sdlc-build-next`,
`/sdlc-validate`, `/sdlc-approve`, `/sdlc-status`, `/sdlc-resume`, and the
rest — full list in `plugins/sdlc-rstack/commands/`), plus the
orchestrator/builder/validator team (`plugins/sdlc-rstack/agents/`). Every
command shells out to `npx rstack-bridge <tool> '<json>'` — the exact same
bridge and harness (`src/core/harness/`) that back the Pi extension, so
there is no separate SDLC logic to drift. `init --framework claude-code`
still owns enforcement (the guard/observe/context hooks below); the plugin
only adds the command surface — see Runtime surfaces for the split.

The marketplace manifest at `.claude-plugin/marketplace.json` lists every
plugin under `plugins/` (generated by `node scripts/generate-marketplace.mjs`
from each plugin's `plugin.json` — regenerate after adding or removing a
plugin), so `/plugin marketplace add` unblocks the whole catalog, not just
this one. `rstack-agents doctor --framework claude-code` verifies both
manifests are present and that the marketplace actually lists `sdlc-rstack`.
...
The `sdlc-rstack` plugin (above) is the supported way to get RStack
subagents and slash commands natively in Claude Code — install it rather
than copying files by hand. `init` itself still does not write
`.claude/agents/rstack/` or `.claude/commands/rstack/` project-local copies,
and there is no `add agents`/`add commands` verb; if you want the broader
196-agent roster (beyond the orchestrator/builder/validator the plugin
ships) exposed the same way, copy those markdown files manually — the
copies are unmanaged (nothing updates or validates them on package
upgrade).
...
| `.claude-plugin/marketplace.json` | Marketplace manifest — `/plugin marketplace add richard-devbot/SDLC-rstack` resolves every plugin under `plugins/` |
| `plugins/sdlc-rstack/` | The installable `/sdlc-*` command surface + orchestrator/builder/validator team (`/plugin install sdlc-rstack`) |
