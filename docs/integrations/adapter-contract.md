...
- Fail open **only** when `npx` itself is missing on the host — the guard can
  never run, and refusing to load the adapter would be worse than skipping
  enforcement for that session. Any other guard outcome is honored as-is.
